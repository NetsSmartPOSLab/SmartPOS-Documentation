...
|`AID`|The "Application ID"|Value "84", and if this is `null` then "4F", from `tags`|
|`TVR`|The "Terminal Verification Result|Value "94" from `tags`|
...
|`applicationLabel`|The application name, if any|`app_label`, and if this is `null` then `card_details`.`scheme`|
...
|`authorisationCode`|Authorisation code|`online_response`.`authorization`.`txnSeqNum`|
|`referenceNumber`|Reference number|`reference_number`|
|`TCC`|The Transaction Condition Code|Calculated based on several values|
|`ARC`|Authorisation response code|Value "8A" from `tags`|
|`acquirer`|The acquirer for this transaction|`host`|
|`TSI`|Transaction status information|Value "9B" from `tags`|
...
merchant ID and currency translation.
